...
#                      gdp_dict=None,
#                      cmap='viridis',
#                      colorbar_label='GDP per capita'):
#     '''
#     Extended version of trophic_plot with GDP-based node coloring option.
#     '''
...
#     # If GDP dict is provided, override node_color with colormap
#     if gdp_dict:
#         nodes = list(G.nodes())
#         values = [gdp_dict.get(node, np.nan) for node in nodes]
#         norm = mcolors.Normalize(vmin=np.nanmin(values), vmax=np.nanmax(values))
#         cmap_func = cm.get_cmap(cmap)
#         node_color = [cmap_func(norm(val)) if not np.isnan(val) else 'lightgray' for val in values]
...
def trophic_plot_gdp(G,
    ...
    if gdp_dict:
        color_values = [gdp_dict.get(node, 0) for node in G.nodes()]
    else:
        color_values = node_color if node_color else [[0, 0.451, 0.7412]] * nnodes
    ...
    nx.draw_networkx(
        G, pos=pos, with_labels=False, ax=ax,
        node_size=scale * 4000,
        arrowsize=scale * 150,
        width=scale * 10,
...
#     ax.set_title(title, fontweight='bold', fontsize=16)
    ...
    ax.set_xlabel(f"Trophic coherence = {1 - F_0:.2f}", fontsize=16)
    ax.set_ylabel("Trophic Levels", fontsize=16)
    ax.tick_params(left=True, labelleft=True)
    
    if gdp_dict:
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=min(gdp_dict.values()), vmax=max(gdp_dict.values())))
        sm.set_array([])
